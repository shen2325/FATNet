...
def save_ckpt_resume(epoch,logdir, model,optimizer, prefix=''):
    state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
    # torch.save(state, os.path.join(logdir, prefix+'model.pth'))
    torch.save({
        'epoch': epoch,
...
def load_ckpt(logdir, model, prefix=''):
    for file in os.listdir(logdir):
        if file.endswith('.pth'):
            ...
            checkpoint_size_mb = os.path.getsize(save_pth) / (1024 * 1024)
            print(f"Checkpoint size: {checkpoint_size_mb:.2f} MB")
            model.load_state_dict(torch.load(save_pth, torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')))
    return model
...
def compute_speed_2(model, input_size, device=0, iteration=100):
    torch.cuda.set_device(device)
    cudnn.benchmark = True
    ...
    input = torch.randn(*input_size, device=device)
    input2 = torch.randn(*input_size, device=device)
...
def compute_speed_yuanzu(model, input_size, device=0, iteration=100):
    torch.cuda.set_device(device)
    cudnn.benchmark = True
