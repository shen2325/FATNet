...
class runningScore(object):
    ...
    def update(self, label_trues, label_preds):
        for lt, lp in zip(label_trues, label_preds):
            # 验证标签值是否在有效范围内
            if np.any(lt < 0) or np.any(lt >= self.n_classes):
                raise ValueError(
                    f"Label true contains values outside the range [0, {self.n_classes - 1}]: {np.unique(lt)}")
            if np.any(lp < 0) or np.any(lp >= self.n_classes):
                raise ValueError(
                    f"Label pred contains values outside the range [0, {self.n_classes - 1}]: {np.unique(lp)}")
    ...
    def get_scores(self):
        ...
        hist = self.confusion_matrix             # 混淆矩阵
        ...
        cls_acc = np.diag(hist) / hist.sum(axis=1)          # 分母为混淆矩阵行方向（label）求和，即：每个真实类别中，所有像素预测为某个类别的总数
