...
class IRSeg(data.Dataset):
    ...
    # 纯黑色，通常用于表示背景或未标注区域。
    # 深紫色（R = 64, G = 0, B = 128），偏暗的紫色，可能用于突出车辆。
    # 橄榄绿色（R = 64, G = 64, B = 0），暗黄色 / 绿色混合，可能用于行人可见性。
    # 蓝绿色（R = 0, G = 128, B = 192），明亮的青色，可能用于区分两轮车辆。
    # 深蓝色（R = 0, G = 0, B = 192），纯蓝色，可能用于标记道路弯曲部分。
    # 暗黄色（R = 128, G = 128, B = 0），类似土黄色，可能用于停车相关区域。
    # 灰紫色（R = 64, G = 64, B = 128），中性色调，可能用于金属或护栏。
    # 粉橙色（R = 192, G = 128, B = 128），柔和的橙粉色，可能匹配实际路锥颜色（如橙色）。
    # 橙红色（R = 192, G = 64, B = 0），鲜艳的橙色 / 红色，可能用于警示减速带。
    @property
    def cmap(self):
        return [
            (0, 0, 0),  # unlabelled
