...
class Resize(object):
    ...
    def __call__(self, sample):
        ...
        for key in sample.keys():
            # BILINEAR for image
            if key in ['image']:
                sample[key] = F.resize(sample[key], self.size, interpolation=InterpolationMode.BILINEAR)
            # NEAREST for depth, label, bound
            else:
                sample[key] = F.resize(sample[key], self.size, interpolation=InterpolationMode.NEAREST)
