...
class RGBXTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1],m1=None,m2=None):
        super().__init__()
        ...
        self.m1=m1
        self.m2=m2
        # if m1 not in models_arr:
        #     raise ValueError(f"Module {m1} is not in the allowed list: {models_arr}")
        #
        # # 检查 m2 是否在列表中
        # if m2 not in models_arr:
    ...
    def forward_features(self, x_rgb, x_e):
        ...
        if self.m2 in m_arr:
            x_fused2 = self.m2_objs[0](x_rgb, x_e)
        ...
        if self.m2 == "CPAM":
            x_fused2 = self.m2_objs[0]([x_rgb, x_e])
        if self.m1 == "GAB":
            mask = torch.mean(x_rgb + x_e, dim=1, keepdim=True)  # 可定义更复杂 mask
            x_fused1 = self.m1_objs[0](x_rgb, x_e, mask=mask)
        if self.m2 == "GAB":
            mask = torch.mean(x_rgb + x_e, dim=1, keepdim=True)  # 可定义更复杂 mask
            x_fused2 = self.m2_objs[0](x_rgb, x_e, mask=mask)
        if self.m1 == "HFF":
            x_fused1 = self.m1_objs[0](
                l=x_rgb,  # 局部特征（来自 RGB 分支）
                g=x_e,  # 全局特征（来自额外模态分支）
                f=None  # 中间特征（可选，例如来自上一层的上采样）
            )
        if self.m2 == "HFF":
            x_fused2 = self.m2_objs[0](
                l=x_rgb,  # 局部特征（来自 RGB 分支）
                g=x_e,  # 全局特征（来自额外模态分支）
                f=None  # 中间特征（可选，例如来自上一层的上采样）
            )
        ...
        if x_fused1 is not None and x_fused2 is not None:
            x_fused = x_fused1 + x_fused2
            # x_fused = torch.abs(torch.sub(x_fused1, x_fused2))
        elif x_fused1 is not None:
            x_fused = x_fused1
        elif x_fused2 is not None:
            x_fused = x_fused2
        else:
            x_fused = x_rgb + x_e

        if self.m2 == "FADC":
            B, C, H, W = x_fused.shape  # 获取原始尺寸
            # print("原始 x_fused 形状:", x_fused.shape)
            # 展平为 (B, H*W, C)
            x_flat = x_fused.view(B, C, H * W).transpose(1, 2)
            # 应用 FFT Transformer Block
            x_transformed = self.m2_objs[0](x_flat)
            # 还原为原始形状 (B, C, H, W)
            x_restored = x_transformed.transpose(1, 2).view(B, C, H, W)
            x_fused = x_restored  # 替换回融合后的输出
        outs.append(x_fused)
        # x_rgb = (x_rgb + x_fused) / 2
        # x_e = (x_e + x_fused) / 2
        ...
        # 直接相加
        if self.m1 in m_arr:
            x_fused1 = self.m1_objs[1](x_rgb, x_e)
        if self.m2 in m_arr:
            x_fused2 = self.m2_objs[1](x_rgb, x_e)

        if self.m1 == "CPAM":
            x_fused1 = self.m1_objs[1]([x_rgb, x_e])
        if self.m2 == "CPAM":
            x_fused1 = self.m2_objs[1]([x_rgb, x_e])
        if self.m1 == "GAB":
            mask = torch.mean(x_rgb + x_e, dim=1, keepdim=True)  # 可定义更复杂 mask
            x_fused1 = self.m1_objs[1](x_rgb, x_e, mask=mask)
        if self.m2 == "GAB":
            mask = torch.mean(x_rgb + x_e, dim=1, keepdim=True)  # 可定义更复杂 mask
            x_fused2 = self.m2_objs[1](x_rgb, x_e, mask=mask)
        if self.m1 == "HFF":
            x_fused1 = self.m1_objs[1](
                l=x_rgb,  # 局部特征（来自 RGB 分支）
                g=x_e,  # 全局特征（来自额外模态分支）
                f=None  # 中间特征（可选，例如来自上一层的上采样）
            )
        ...
        if self.m2 == "FADC":
            ...
            x_flat = x_fused.view(B, C, H * W).transpose(1, 2)
            # 应用 FFT Transformer Block
            x_transformed = self.m2_objs[1](x_flat)
            # 还原为原始形状 (B, C, H, W)
            x_restored = x_transformed.transpose(1, 2).view(B, C, H, W)
            x_fused = x_restored  # 替换回融合后的输出
        outs.append(x_fused)
        ...
        # 直接相加
        if self.m1 in m_arr:
            x_fused1 = self.m1_objs[2](x_rgb, x_e)
        if self.m2 in m_arr:
            x_fused2 = self.m2_objs[2](x_rgb, x_e)

        if self.m1 == "CPAM":
            x_fused1 = self.m1_objs[2]([x_rgb, x_e])
        if self.m2 == "CPAM":
            x_fused1 = self.m2_objs[2]([x_rgb, x_e])

        if self.m1 == "GAB":
            mask = torch.mean(x_rgb + x_e, dim=1, keepdim=True)  # 可定义更复杂 mask
            x_fused1 = self.m1_objs[2](x_rgb, x_e, mask=mask)
        if self.m2 == "GAB":
            mask = torch.mean(x_rgb + x_e, dim=1, keepdim=True)  # 可定义更复杂 mask
            x_fused2 = self.m2_objs[2](x_rgb, x_e, mask=mask)
        if self.m1 == "HFF":
            x_fused1 = self.m1_objs[2](
                l=x_rgb,  # 局部特征（来自 RGB 分支）
                g=x_e,  # 全局特征（来自额外模态分支）
                f=None  # 中间特征（可选，例如来自上一层的上采样）
            )
        ...
        if self.m2 == "FADC":
            ...
            x_flat = x_fused.view(B, C, H * W).transpose(1, 2)
            # 应用 FFT Transformer Block
            x_transformed = self.m2_objs[2](x_flat)
            # 还原为原始形状 (B, C, H, W)
            x_restored = x_transformed.transpose(1, 2).view(B, C, H, W)
            x_fused = x_restored  # 替换回融合后的输出
        outs.append(x_fused)
        ...
        x_e = x_e.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        if self.m1 in m_arr:
            x_fused1 = self.m1_objs[3](x_rgb, x_e)
        if self.m2 in m_arr:
            x_fused2 = self.m2_objs[3](x_rgb, x_e)

        if self.m1 == "CPAM":
            x_fused1 = self.m1_objs[3]([x_rgb, x_e])
        if self.m2 == "CPAM":
            x_fused1 = self.m2_objs[3]([x_rgb, x_e])

        if self.m1 == "GAB":
            mask = torch.mean(x_rgb + x_e, dim=1, keepdim=True)  # 可定义更复杂 mask
            x_fused1 = self.m1_objs[3](x_rgb, x_e, mask=mask)
        if self.m2 == "GAB":
            mask = torch.mean(x_rgb + x_e, dim=1, keepdim=True)  # 可定义更复杂 mask
            x_fused2 = self.m2_objs[3](x_rgb, x_e, mask=mask)
        if self.m1 == "HFF":
            x_fused1 = self.m1_objs[3](
                l=x_rgb,  # 局部特征（来自 RGB 分支）
                g=x_e,  # 全局特征（来自额外模态分支）
                f=None  # 中间特征（可选，例如来自上一层的上采样）
            )
        ...
        if self.m2 == "FADC":
            ...
            x_flat = x_fused.view(B, C, H * W).transpose(1, 2)
            # 应用 FFT Transformer Block
            x_transformed = self.m2_objs[3](x_flat)
            # 还原为原始形状 (B, C, H, W)
            x_restored = x_transformed.transpose(1, 2).view(B, C, H, W)
            x_fused = x_restored  # 替换回融合后的输出
        outs.append(x_fused)
        outs_t, outs_2 = None, None
        if self.m1 == "AFPN":
            out0, out1, out2, out3 = self.m1_objs(outs)
            outs_t = [out0, out1, out2, out3]
        if self.m2 == "AFPN":
            out0, out1, out2, out3 = self.m2_objs(outs)
            outs_t = [out0, out1, out2, out3]
        if self.m1 == "DMCfusion":
            outs_2 = self.m1_objs(outs[0],  outs[1], outs[2], outs[3])
        if self.m2 == "DMCfusion":
            outs_2 = self.m2_objs(outs[0],  outs[1], outs[2], outs[3])
        if outs_t is not None and outs_2 is not None:
            outs = outs_t+ outs_2
        elif outs_t is not None:
            outs = outs_t
        elif outs_2 is not None:
            outs = outs_2
...
class mit_b2(RGBXTransformer):
    def __init__(self, fuse_cfg=None,m1=None,m2=None, **kwargs):
        super(mit_b2, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1,m1=m1,m2=m2)
...
if __name__ == '__main__':
    ...
    model = mit_b2(norm_fuse=nn.BatchNorm2d,m1="BFAM",m2="SDM2d")
