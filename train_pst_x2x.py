...
from toolbox import ClassWeight, save_ckpt_resume
...
def run(args):
    ...
    for ep in range(start_epoch,cfg['epochs']):
        ...
        for i, sample in enumerate(train_loader):
            ...
            cur_t = time.time()
            if cur_t-t > 5:
                logger.info('|- epo %s/%s. train iter %s/%s. %.2f img/sec loss: %.4f, acc: %.4f' \
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config", type=str, default="models_rgbx_diy_x_2_x/pst_rgbx.json", help="Configuration file to use")
